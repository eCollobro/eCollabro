...
function getCommentList(commentList) {
    ...
    for (var ctr = 0; ctr < commentList.length; ctr++) {
        ...
        list += "<img class='media-object' data-src='holder.js/64x64' alt='" + addedBy + "' src='data:image/svg+xml;base64,' style='width: 64px; height: 64px;'>";
        list += "</a>";
...
ecollabroApp.controller('contentCommentsController', ['$scope', 'contentCommentService', function ($scope, contentCommentService) {
    ...
    //Method setCommentFlags
    $scope.setCommentFlags = function (isCommentAllowed, isLikeAllowed, isRatingAllowed, isVotingAllowed) {
    ...
    $scope.loadComments = function () {
        ...
        contentCommentService.getContentComments($scope.contextId, $scope.contextContentId).then(function (resp) {
            if (resp.businessException == null) {
                $scope.contentCommentsData = resp.result;
                $scope.userRating =''+ $scope.contentCommentsData.UserRating;
                if ($scope.isCommentAllowed) {
                    list = "";
                    getCommentList($scope.contentCommentsData.ContentComments);
    ...
    //Method setLikeDislike
    $scope.setLikeDislike = function () {
         contentCommentService.changeContentLikeDislike($scope.contextId,$scope.contextContentId,true).then(function (resp) {
            if (resp.businessException == null) {
                $scope.loadComments();
            }
            else {
    ...
    $scope.setVote = function () {
        contentCommentService.changeContentVote($scope.contextId, $scope.contextContentId, true).then(function (resp) {
            if (resp.businessException == null) {
                $scope.loadComments();
            }
            else {
    ...
    $scope.setRating = function () {
        contentCommentService.changeContentRating($scope.contextId, $scope.contextContentId, $scope.userRating).then(function (resp) {
            if (resp.businessException == null) {
                $scope.loadComments();
            }
            else {
