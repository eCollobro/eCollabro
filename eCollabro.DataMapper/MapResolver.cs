...
namespace eCollabro.DataMapper
{
    #region MapResolver
            ...
            if (!maps.TryGetValue(key, out map))
            {
                lock (mutex)
                {
                    if (!maps.TryGetValue(key, out map))
                    {
                        ...
                        if (options==MapResolveOptions.UsePrivateSetter)
                        {
                            getProperties= GetProperties(sourceType, BindingFlags.Public | BindingFlags.Instance | BindingFlags.GetProperty | BindingFlags.NonPublic);
                        }
                        else
                        {
                            getProperties = GetProperties(sourceType, BindingFlags.Public | BindingFlags.Instance | BindingFlags.GetProperty );
                        }

                        //// set properties in destination type
                        IEnumerable<PropertyInfo> setProperties;
                        if (options == MapResolveOptions.UsePrivateSetter)
                        {
                            setProperties = GetProperties(destinationType, BindingFlags.Public | BindingFlags.Instance | BindingFlags.SetProperty | BindingFlags.NonPublic);
                        }
                        else
                        {
                            setProperties = GetProperties(destinationType, BindingFlags.Public | BindingFlags.Instance | BindingFlags.SetProperty );
                        }
